...
public sealed class SignInCommandHandlerTests
{
    ...
    public async Task Handle_IdentityServiceInvalidPayloadResult_ReturnsIdentityServiceResult()
    {
        ...
        var requestObject = _fixture.Create<SignInCommand>();
        _identityService.Setup(x => x.SignInUser(It.IsAny<SignInCommand>())).ReturnsAsync(resultObject);
        var handler = new SignInCommandHandler(_identityService.Object);
    ...
    [Fact]
    public async Task Handle_IdentityServiceServerErrorResult_ReturnsIdentityServiceResult()
    {
    ...
    [Fact]
    public async Task Handle_IdentityServiceIncorrectEmailOrPasswordResult_ReturnsIdentityServiceResult()
    {
        var resultObject = AuthResultDtoFactory.IncorrectEmailOrPasswordError();
        var requestObject = _fixture.Create<SignInCommand>();
        _identityService.Setup(x => x.SignInUser(It.IsAny<SignInCommand>())).ReturnsAsync(resultObject);
        var handler = new SignInCommandHandler(_identityService.Object);
